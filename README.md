...
### 4. 修复 Windows 下上传文件大小校验失败的问题 (Code 1062009)

- **问题**: 在 Windows 环境下，由于换行符 (`\r\n`) 在文本读取时被 Python 自动转换，导致 `os.path.getsize` 获取的物理大小与实际上传内容的大小不一致，触发飞书 API 的 `size inconsistent` 校验错误。
- **修复**: 在上传前对 Markdown 内容进行换行符归一化处理（统一为 `\n`），并利用 `io.BytesIO` 以流式方式提交归一化后的字节数据，确保字节长度与声明大小严格一致。

### 5. 增强图片上传的稳定性与自动清理机制

- **网络抖动处理**: 针对图片上传和文档块更新环节，增加了**指数退避重试机制**（最大 3 次重试），有效解决了因网络波动或接口瞬时压力导致的 `Expecting value: line 1 column 1` (JSON 解析异常) 错误。
- **失败自动回滚**: 实现了导入失败后的自动清理逻辑。若在迁移过程中发生错误，程序会自动识别并删除已上传的临时 `.md` 文件（类型：`file`）以及已创建的半成品云文档（类型：`docx`），避免在云端产生废弃残留。
